...
Index building can be parallelized, both for `[build.internal]` and `[build.external]`. Optimize parallelism using the following settings:
...
If you are using `[build.internal]`, you must also set `build_threads` in the `CREATE INDEX` command.
```sql
