...
When migrating from `PGVecto.rs` to `VectorChord`, it is possible that you have a table with a column of vector type. Suppose you have `PGVecto.rs` installed. An example would be:
...
```sql
SELECT vec_f32, pg_typeof("vec_f32") from table_vector limit 1;
--    vec_f32    | pg_typeof 
-----------------+-----------
-- [0.1,0.1,0.1] | vector
```

### Appendix: about cast of sparse vector
